...
/*!50003 DROP PROCEDURE IF EXISTS `resolve_alias` */;
...
set @first_dash = position('-' in inmail);
...
IF (@length - (@first_dash - @last_at)) > 0
	THEN
		...
		set @user = substring_index(inmail, '-', 1);
		...
		or
		(`requires_identifier` = 1 and @identifier != '' and @identifier REGEXP '[a-z,0-9]+' and `source` = concat(@user, '@', @domain));
	ELSE
		SELECT `destination`
		FROM `virtual_aliases`
		WHERE
...
-- Dump completed on 2025-02-16  9:23:06
